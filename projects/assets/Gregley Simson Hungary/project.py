...
class Project(Document):
	...
	def after_insert(self):
		#client :gergely simon. create a new repair sheet doctype and auto assign
		create_repair_sheet(self.name,self.assigned_user)
		#client :gergely simon. create a new repair sheet doctype and auto assign
		if self.sales_order:
...
# auto create Engine Repair Sheet when Project type is Engine Repair
def create(project):
...
#client :gergely simon. create a new repair sheet doctype and auto assign
def create_repair_sheet(project_name,assigned_user):
	if not frappe.db.exists("Engine Repair Sheet", project_name):
		tps = frappe.new_doc("Engine Repair Sheet")
		tps.flags.ignore_mandatory = True
		tps.project = project_name
		tps.insert()
		project_detail="Project : {0}".format(project_name)
		engine_repair_detail=" Repair Sheet:{0}".format(tps.name)+"\n"
		main=project_detail+engine_repair_detail
		url = get_url("/Engine Repair Sheet/?name={0}".format(tps.name))
		messages = (
		_("You have been assigned Engine Repair Sheet: {0}".format(tps.name)),
		url,
		_("Click Link")
		)
		...
		<p><a href="{1}">{2}</a></p>
		"""
		assign_tasks("Engine Repair Sheet",assigned_user,tps.name,content=content.format(*messages))
