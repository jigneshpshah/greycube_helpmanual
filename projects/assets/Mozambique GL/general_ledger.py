...
#running total
	...
	for d in data:
		if d.get("account") is not None:			
			if  ((d.get("account").find("Opening") ==-1) and (d.get("account").find("Total") ==-1)and (d.get("account").find("Balance") ==-1)):
				diff=d.get("credit")-d.get("debit")
				holder=holder+diff
				if holder>=0:
					showval="<div align='right'>"+"<font color='green'>"+str(holder)+"</font>"+"</div>"
				else:
					showval="<div align='right'>"+"<font color='red'>"+str(holder)+"</font>"+"</div>"
			else:
				showval=""
		row = [d.get("posting_date"), d.get("account"), d.get("debit"), d.get("credit"),showval]
#running total
		if filters.get("show_in_account_currency"):
